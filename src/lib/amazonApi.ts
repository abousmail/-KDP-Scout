...
// FNV-1a — deterministic variance, never Math.random()
...
/**
 * Calls /api/suggestions (Vercel Edge proxy → Amazon → Google fallback).
 * Returns suggestions + the data source actually used.
 * Throws on network/HTTP error — callers should catch and use mock data.
 */
export async function fetchSuggestions(
  ...
  return { suggestions, source: data.source ?? 'amazon' };
}

/**
 * Converts a suggestions list into KeywordResult[] with estimated volume/competition.
 * Amazon/Google sort suggestions by search frequency → position encodes volume rank.
 */
